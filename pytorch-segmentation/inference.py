...
def save_probability_matrix_numpy(prob_matrix, file_path):
    prob_matrix_np = prob_matrix.cpu().numpy()  # 將 PyTorch tensor 轉換為 NumPy 數組
    np.save(file_path, prob_matrix_np)  # 保存為 NumPy 文件
...
def main():
    ...
    with torch.no_grad():
        ...
        for img_file in tbar:
            ...
            # 保存概率矩陣
            prob_matrix_file = os.path.join(args.output, os.path.basename(img_file).split('.')[0] + '.npy')
            save_probability_matrix_numpy(torch.from_numpy(prediction), prob_matrix_file)
