...
def compute_silhouette_per_class(silhouette_values, labels, num_classes):
    ...
    for i in range(num_classes):
        # Get silhouette values for samples of class i
...
def inference(data_root, model_name, model_path, output_dir, batch_size):
    ...
    with torch.no_grad():
        for inputs, labels in tqdm(dataloader, desc="Inference", unit="batch"):
            ...
            #     features = model.avgpool(model.inception5b(model.inception5a(model.inception4e(model.inception4d(model.inception4c(model.inception4b(model.inception4a(model.inception3b(model.inception3a(model.maxpool1(model.conv1(inputs))))))))))))
    ...
    # Add silhouette scores to results
    ...
    # ----------------------------------Save silhouette scores and distances to separate CSV files----------------------------------
    silhouette_scores = {
